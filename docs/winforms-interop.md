...
| Form granularity | This bridge exchanges whole forms/windows. To embed Majorsilence.Forms **controls** inside existing WinForms UI (port one control at a time, or keep a control library's WinForms surface while its internals migrate), use the `Majorsilence.Forms.WinForms` backend's `MajorsilenceFormsPresenter`/`ToWinFormsControl()` instead — see [Platform backends](backends.md#the-winforms-backend) and `samples/EmbeddingWinForms`. |
