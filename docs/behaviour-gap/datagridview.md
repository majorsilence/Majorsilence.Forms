...
`DGV-03`, `DGV-31`, `DGV-32`, `DGV-33` are closed as of 2026-09-13 (`W5.3`): `OnBoundListChanged`
honours the change type, a `Reset` regenerates columns only when the schema differs, `Rows.Add ()`
returns the new index *and* gives the row its cells, `DataSource` returns what was assigned,
...
### DGV-03 — `DataGridViewRowCollection.Add()` / `Add(int count)` return value — Cat A — P0 — High — **CLOSED 2026-09-13 (W5.3)**
...
### DGV-31 — Data binding: every `ListChanged` rebuilds all columns and rows — Cat A — P0 — High — **CLOSED 2026-09-13 (W5.3)**
...
### DGV-32 — `DataMember` ignored; `DataSource` getter returns the resolved list; unsupported sources silently keep the old list — Cat A — P1 — High — **CLOSED 2026-09-13 (W5.3)**
...
### DGV-33 — `RowsAdded` not raised for bound rows or `ReplaceAll`; `RowsRemoved` raised on rebind; `Rows.Insert(int,row)` bypasses `RowsAdded`; `Rows.Add*` while bound does not throw — Cat D/A — P1 — High — **CLOSED 2026-09-13 (W5.3)**
...
## Systemic patterns
- **`add { } remove { }` events discard the subscriber** even where the framework already has the trigger point in hand. `CellMouseDown/Up/Move/DoubleClick`, `RowHeaderMouseClick`, `CellContentDoubleClick`, `ColumnHeaderMouseDoubleClick` and `SortCompare` are closed (W5.4/W5.5); `ColumnWidthChanged`, `RowHeightChanged`, `ColumnHeadersHeightChanged`, `RowHeadersWidthChanged`, `ColumnSortModeChanged`, `AutoSizeColumnModeChanged` are closed (W6.1, 2026-09-14) — each backed with a field + `On*` raiser, called from the obvious site (`Column.Width` setter → `ColumnWidthChanged`, `Row.Height` setter → `RowHeightChanged`, `ColumnHeadersHeight` setter → `ColumnHeadersHeightChanged`, etc.). **Still open, and not a one-line sweep:** `RowStateChanged`/`CellStateChanged` (the trigger point, `Selected`, is real, but the batch selection paths write `SetSelectedCore` directly and would silently not fire either event — see W6.1's note in the plan), `DefaultValuesNeeded`, `UserAddedRow`, `NewRowNeeded` (all need the new-row placeholder, `DGV-05`), `CellValueNeeded`/`CellValuePushed` (need `VirtualMode` to be a real mode, not a stored property), `ColumnDisplayIndexChanged` (needs column reordering, which `DisplayIndex`'s setter does not do).
...
- **Row/column/cell objects are passive auto-properties**: `Cell.Value`, `Cell.ErrorText`, `Column.DisplayIndex`, `HeaderCell.SortGlyphDirection`, `HeaderCell.Value`, `RowTemplate` change nothing because no setter calls back into the owning grid. (`Row.Visible`, `Row.Selected`, `Cell.Selected` and `Column.SortMode` are no longer in this list — W5.2b and W6.1 gave each a hook.) Give the rest a `owner?.DataGridView?.On…` hook the way `Column.Width`/`Row.Height` already do.
