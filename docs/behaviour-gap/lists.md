...
- **MOSTLY CLOSED (2026-09-16).** The band, the membership and the collapse are real: `LayoutItems` places a one-row header band per group followed by that group's items; `ListViewRenderer.RenderGroupHeader` paints the caption and a rule; `CollapsedState.Collapsed` lays a group's items at `Rectangle.Empty`, which takes them out of painting and every hit-test at once rather than teaching each one about collapse; `ListViewItem.Group` is typed `ListViewGroup?` and keeps `group.Items` in step both ways; `ListViewGroupCollection` sets `ListViewGroup.ListView` on insert and clears it on removal; and `LineCount` counts the bands, so a grouped list can still scroll to its last row. Ungrouped items run last **without** a band — dropping them would make grouping *some* items silently hide the rest, which is a worse failure than the one being fixed.
- **Deliberate simplification:** a band is exactly one row tall. Upstream's are taller; matching that means teaching `top_index`/`ScaledLineHeight`/`VisibleLineCount` and the scrollbar about variable line heights, which is a separate job. As it stands every line is still one row and there are simply more of them, so none of the scrolling arithmetic changed.
