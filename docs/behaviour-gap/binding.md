...
- **Events with a natural trigger that is never pulled.** `BindingSource.PositionChanged`, `BindingComplete`, `DataError`, `CurrencyManager.ItemChanged/ListChanged/MetaDataChanged`, `BindingContext.CollectionChanged` — all declared, all have an `On*` or an obvious call site, none called. (`Control.BindingContextChanged` is no longer in this list — its setter call site is wired, W6.1 — but nothing subscribes to it, so BND-15's re-homing is still not happening.)
