...
namespace Modern.Forms.Tests
{
    ...
    // Modern.Forms API. Modern.Forms exposes FolderBrowserDialog as a plain FileSystemDialog subclass
    // with no Win32 plumbing, so only the cross-platform behavioral surface that actually exists is
    // pinned here. Upstream members that Modern.Forms does not implement (AddToRecent,
    ...
    public class FolderBrowserDialogTests
    {
        ...
        {
            // Modern.Forms coerces null to an empty string; the getter never returns null.
