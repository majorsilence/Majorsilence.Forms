...
namespace Modern.Forms.Tests
{
    ...
    // Modern.Forms compat layer. These exercise CONTROL-level behavior (the Columns/Rows
    ...
    //
    // Modern.Forms is a compat layer: many of these properties are simple auto-properties or
    // value-changed setters with no element-state plumbing, layout callbacks, or handle creation,
    // so the assertions pin Modern.Forms' ACTUAL semantics (which differ in defaults from the
