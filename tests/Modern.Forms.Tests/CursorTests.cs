...
namespace Modern.Forms.Tests
{
    ...
    // Modern.Forms API. Modern.Forms cursors are backend-neutral value descriptors built around a
    ...
    public class CursorTests
    {
        ...
        {
            // Modern.Forms caches each backing cursor (??=), so repeated reads of a property must
        ...
        public void Cursors_ToString_KnownCursor_ReturnsExpected ()
        {
            ...
            foreach (var property in typeof (Cursors).GetProperties (BindingFlags.Static | BindingFlags.Public)) {
                ...
                Assert.NotEqual ("Modern.Forms.Cursor", cursor!.ToString ());
