...
namespace Modern.Forms.Tests
{
    ...
    // Modern.Forms compat layer. Modern.Forms exposes most DataGridViewCell members as simple
    // auto-properties (no element-state plumbing, accessibility, edit controls, or shared rows),
    // so these pin the actual Modern.Forms semantics: constructor defaults, Value get/set with the
    ...
    //
    // Where Modern.Forms intentionally diverges from WinForms (e.g. ReadOnly does not throw when the
    ...
    // tests assert the real Modern.Forms behavior rather than the WinForms behavior.
    public class DataGridViewCellTests
    {
        ...
        {
            // Unlike WinForms (which throws when the cell has no owning row), Modern.Forms ReadOnly
        ...
        {
            // Modern.Forms semantics: setting the row's ReadOnly cascades to every cell.
            var row = new DataGridViewRow ();
            var cell = new DataGridViewCell ();
            row.Cells.Add (cell);
