    ...
    public sealed class CoordinateSpaceTests : IDisposable
    {
        ...
        public void ListView_HitTest_takes_a_logical_point ()
        {
            ...
            try {
                ...
                var bounds = view.DeviceToLogicalUnits (view.Items[1].Bounds);
                var point = new Point (bounds.Left + 5, bounds.Top + bounds.Height / 2);

                Assert.Same (view.Items[1], view.HitTest (point).Item);

                // And the bounds really are scaled, or the conversion above would be doing nothing.
                Assert.True (view.Items[1].Bounds.Top > bounds.Top, "the fixture must actually be scaled");
            } finally {
                form.Close ();
            }
        }
