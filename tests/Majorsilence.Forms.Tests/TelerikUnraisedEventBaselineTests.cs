    ...
    public void NoNewUnraisedEvents ()
    {
        ...
        if (Environment.GetEnvironmentVariable ("MAJORSILENCE_WRITE_UNRAISED_EVENT_BASELINE") == "1") {
            ...
            StubSurfaceScanner.WriteBaseline (baselinePath, [
                ...
                "#",
                $"# {actual.Count} of {examined} field-backed events ({subscribable} events are subscribable in all; the rest are",
                ...
                $"#    reaches the raiser. Over this assembly it finds {singleHop.Count} where this one finds {actual.Count}, and the",
