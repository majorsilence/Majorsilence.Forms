...
namespace Majorsilence.Forms.Tests
{
    ...
    // Majorsilence.Forms compat layer. Majorsilence.Forms exposes most DataGridViewCell members as simple
    // auto-properties (no element-state plumbing, accessibility, edit controls, or shared rows),
    // so these pin the actual Majorsilence.Forms semantics: constructor defaults, Value get/set with the
    ...
    //
    // Where Majorsilence.Forms intentionally diverges from WinForms (e.g. ReadOnly does not throw when the
    ...
    // tests assert the real Majorsilence.Forms behavior rather than the WinForms behavior.
    public class DataGridViewCellTests
    {
        ...
        {
            // Unlike WinForms (which throws when the cell has no owning row), Majorsilence.Forms ReadOnly
        ...
        {
            // Majorsilence.Forms semantics: setting the row's ReadOnly cascades to every cell.
            var row = new DataGridViewRow ();
            var cell = new DataGridViewCell ();
            row.Cells.Add (cell);
