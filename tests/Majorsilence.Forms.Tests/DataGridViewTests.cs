...
namespace Majorsilence.Forms.Tests
{
    ...
    // Majorsilence.Forms compat layer. These exercise CONTROL-level behavior (the Columns/Rows
    ...
    //
    // Majorsilence.Forms is a compat layer: many of these properties are simple auto-properties or
    // value-changed setters with no element-state plumbing, layout callbacks, or handle creation,
    // so the assertions pin Majorsilence.Forms' ACTUAL semantics (which differ in defaults from the
    ...
    public class DataGridViewTests
    {
        ...
        [Fact]
        public void AutoGeneratedColumns_from_empty_typed_list_exist_and_survive_HeaderText_rename ()
        {
            // A common designer pattern binds a grid to an (often empty) typed list, then configures
            // columns by field name AND renames their headers. Columns must be generated from the
            // element type's schema even when there are no rows, and stay findable by field name after
            // the header is reassigned (Columns[name] matches Name, not just HeaderText).
            using var grid = new DataGridView { DataSource = new System.Collections.Generic.List<Reminder> () };

            Assert.NotNull (grid.Columns["Summary"]);

            grid.Columns["Summary"]!.HeaderText = "Notes";

            // Still resolvable by the original member name after the header changed.
            Assert.NotNull (grid.Columns["Summary"]);
            Assert.Equal ("Notes", grid.Columns["Summary"]!.HeaderText);
        }

        [Fact]
        public void AutoGeneratedColumns_from_empty_DataView_exist_and_survive_HeaderText_rename ()
        {
            using var table = new System.Data.DataTable ();
            table.Columns.Add ("Start", typeof (System.DateTime));
            table.Columns.Add ("Description", typeof (string));

            using var grid = new DataGridView { DataSource = table.DefaultView }; // zero rows

            Assert.NotNull (grid.Columns["Start"]);

            grid.Columns["Start"]!.HeaderText = "Reminder Date";

            Assert.NotNull (grid.Columns["Start"]);
        }

        [Fact]
        public void AutoGeneratedColumns_from_BindingSource_over_DataSet_DataMember ()
        {
            // The grid is often bound to a BindingSource whose DataSource is a DataSet and DataMember
            // names a table. BindingSource exposes that table's schema via ITypedList, so the grid must
            // generate columns from it (by column name), not from the DataRowView's CLR properties.
            using var set = new System.Data.DataSet ();
            var table = set.Tables.Add ("arCustReceiptsSmry");
            table.Columns.Add ("ReceiptNum", typeof (int));
            table.Columns.Add ("RecTotal", typeof (decimal));
            table.Rows.Add (1, 10.5m);

            using var bs = new BindingSource { DataSource = set, DataMember = "arCustReceiptsSmry" };
            using var grid = new DataGridView { DataSource = bs };

            Assert.NotNull (grid.Columns["ReceiptNum"]);
            Assert.NotNull (grid.Columns["RecTotal"]);

            grid.Columns["ReceiptNum"]!.HeaderText = "Receipt Num";
            Assert.NotNull (grid.Columns["ReceiptNum"]); // still findable by field name after rename
        }

        [Fact]
        public void Binding_a_source_with_rows_seeds_CurrentRow ()
        {
            // WinForms/Telerik make the first row current once a non-empty source is bound; app code
            // reads grid.CurrentRow.Cells[...] right after binding, so it must not be null.
            using var grid = new DataGridView {
                DataSource = new System.Collections.Generic.List<Reminder> {
                    new () { Id = 1, Summary = "a" },
                    new () { Id = 2, Summary = "b" },
                }
            };
