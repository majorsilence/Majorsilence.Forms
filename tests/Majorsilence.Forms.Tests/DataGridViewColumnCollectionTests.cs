...
namespace Majorsilence.Forms.Tests
{
    ...
    // to the Majorsilence.Forms compat layer. The upstream suite leans heavily on element-state plumbing
    // (DisplayIndex reordering, frozen/auto-size validation, IsHandleCreated, ISupportInitialize)
    // that Majorsilence.Forms intentionally does not model, so those cases are omitted. What remains pins
    // the collection mechanics Majorsilence.Forms actually implements: Add/AddRange/Insert/Remove/RemoveAt/
