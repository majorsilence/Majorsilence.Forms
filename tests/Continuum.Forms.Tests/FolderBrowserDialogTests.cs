...
namespace Continuum.Forms.Tests
{
    ...
    // Continuum.Forms API. Continuum.Forms exposes FolderBrowserDialog as a plain FileSystemDialog subclass
    // with no Win32 plumbing, so only the cross-platform behavioral surface that actually exists is
    // pinned here. Upstream members that Continuum.Forms does not implement (AddToRecent,
    ...
    public class FolderBrowserDialogTests
    {
        ...
        {
            // Continuum.Forms coerces null to an empty string; the getter never returns null.
