...
namespace Continuum.Forms.Tests
{
    ...
    // Continuum.Forms API. SystemInformation in Continuum.Forms is a cross-platform stub provider: most
    // metrics are fixed constants rather than OS-queried, so these tests pin the exact constants
    // Continuum.Forms returns. The screen-derived members (WorkingArea, PrimaryMonitorSize,
    ...
    public class SystemInformationTests
    {
        ...
        {
            // Upstream asserts width > 0; Continuum.Forms pins it to a fixed value.
        ...
        {
            // Continuum.Forms exposes this as a single int (not a Size, unlike WinForms).
