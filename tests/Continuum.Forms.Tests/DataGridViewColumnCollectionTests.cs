...
namespace Continuum.Forms.Tests
{
    ...
    // to the Continuum.Forms compat layer. The upstream suite leans heavily on element-state plumbing
    // (DisplayIndex reordering, frozen/auto-size validation, IsHandleCreated, ISupportInitialize)
    // that Continuum.Forms intentionally does not model, so those cases are omitted. What remains pins
    // the collection mechanics Continuum.Forms actually implements: Add/AddRange/Insert/Remove/RemoveAt/
