...
namespace Continuum.Forms.Tests
{
    ...
    // Continuum.Forms API. Continuum.Forms cursors are backend-neutral value descriptors built around a
    ...
    public class CursorTests
    {
        ...
        {
            // Continuum.Forms caches each backing cursor (??=), so repeated reads of a property must
        ...
        public void Cursors_ToString_KnownCursor_ReturnsExpected ()
        {
            ...
            foreach (var property in typeof (Cursors).GetProperties (BindingFlags.Static | BindingFlags.Public)) {
                ...
                Assert.NotEqual ("Continuum.Forms.Cursor", cursor!.ToString ());
