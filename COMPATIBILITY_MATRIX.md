...
| `Button`, `CheckBox`, `RadioButton`, `Label`, `LinkLabel`, `PictureBox`, `Panel`, `GroupBox`, `TabControl`/`TabPage`, `FlowLayoutPanel`, `TableLayoutPanel`, `TrackBar`, `ProgressBar`, `ScrollBar`/`HScrollBar`/`VScrollBar`, `UserControl` | Implemented | Only the systemic gaps above; no missing members specific to these types. They pick up the whole `Control` protected surface by inheritance. `Button.TextAlign`/`ImageAlign` default to `MiddleCenter` as WinForms' `ButtonBase` does (as of 2026-08-07); `CheckBox`/`RadioButton` keep WinForms' `MiddleLeft`. `Label.AutoSize` really measures the text and resizes to it, growing *and* shrinking (as of 2026-08-07): `GetPreferredSize` used to return the size the label already had, so an auto-sized label kept whatever the designer left on it — and an over-wide label is opaque to the mouse, so it swallows the clicks of the container underneath. |
| `TextBox` | Implemented | Full surface for get/set/select/undo usage. `BorderStyle` and `TextAlign` drive rendering rather than only storing a value (as of 2026-08-07), and a single-line box centres its text vertically the way a Win32 `EDIT` without `ES_MULTILINE` does; multiline still starts at the top. As of 2026-09-03 (`W5.11`) five more properties do something: `CharacterCasing` converts typed, pasted and assigned text; `ShortcutsEnabled = false` refuses Ctrl+C/X/V/A; `WordWrap = false` stops a multiline box wrapping and gives it a horizontal scrollbar (except centre/right-aligned, which still wraps — the layout engine needs a right edge to align against); `ScrollBars` decides whether a bar appears at all, where previously **no** box ever showed one, because a `new` shadow hid the working base property; and `HideSelection` decides whether the selection is *painted*, the selection itself now surviving focus loss so an Edit menu or Find dialog can still act on it. Two crashes fixed with them: typing into a box whose text is already longer than `MaxLength` (`TXT-05`), and the caret walking into `PlaceholderText` and taking the next keystroke down with it (`TXT-06`). `AppendText` writes through the document rather than `Text +=` as of 2026-09-03 (`W5.12`), so a log window keeps its scroll position, its undo history and its `Modified` flag instead of jumping to the oldest line on every append (`TXT-02`, P0). Undo works when called but is unreachable from the keyboard — Ctrl+Z is not bound in the key chain (`TXT-13`, noted 2026-08-31). |
| `RichTextBox` | Partial | Real document behaviour as of 2026-09-03 (`W5.14`). **`Rtf` renders the current text** — it used to return a string only its own setter ever wrote, so `note.Body = rtb.Rtf` saved an empty document over whatever the user had typed (`TXT-04`, P0). The RTF reader keeps paragraphs (`\par` → newline), tabs, `\'xx` and `\uN` escapes and text at any group depth, skipping metadata destinations by name; it previously kept only depth-1 text, so accented characters, paragraph breaks and anything inside a `{...}` run were lost on load and then saved back (`TXT-15`). `LoadFile`/`SaveFile` default to `RichText` and honour `fileType` (`TXT-16`). `Find` is case-insensitive by default, honours `MatchCase`/`WholeWord`/`Reverse`/`NoHighlight`, treats `end == -1` as the end of the text, throws for an unsearchable range, and **selects and scrolls to the hit** — without which the standard highlight-every-occurrence loop had nothing to colour (`TXT-14`). `SelectionColor`, `SelectionBackColor`, `SelectionBold`, `SelectionItalic`, `SelectionUnderline` and the style flags of `SelectionFont` are real per-run formatting, applied to the selection or to what is typed/appended next, and painted (`TXT-17`). **Still missing:** character formatting is not written into the generated RTF (text and paragraphs survive a save; colours do not), `SelectionFont` has no per-run family or size, and runs do not follow `Undo`, `Paste` or a programmatic `SelectedText` assignment. Also no `Undo`/`Redo`/`CanUndo`/`CanRedo`/`RedoActionName`, no `SelectedRtf`, no `CanPaste`, no `AutoWordSelection`, and `ScrollBars` is still a `new` shadow and still stored-only. The `AppendText`/`SelectedText` shadows were removed by `W5.12` (`TXT-02` P0, `TXT-35`). |
...
| `NumericUpDown`, `DomainUpDown` | Partial | No `BeginInit`/`EndInit` (`ISupportInitialize`), no `BorderStyle`, no `ParseEditText`/`UpdateEditText` overrides. Fixed 2026-09-04 (`W5.20a`): the arrows step by `Increment` — they call `UpButton`/`DownButton`, the methods that apply it, where the mouse path used to hard-code ±1, so a control set to 0.01 for currency or 5 for quantities moved by 1 under the user's finger (`SMP-31`, P0). **Still missing:** `NumericUpDown` has no keyboard input at all, so a number cannot be typed into it (`SMP-32`, P0); `DomainUpDown` derives from it and renders a number instead of its `Items` (`SMP-37`); and structurally neither derives from `UpDownBase` as upstream does (`SMP-36`), which is ordered before the text-entry work so it does not land twice. |
| `ScrollBar`, `HScrollBar`, `VScrollBar` | Implemented | Real as of 2026-09-04 (`W5.20a`). A user-driven scroll stops at `Maximum - LargeChange + 1`, upstream's rule and the reason `Maximum` is conventionally set past the last item: reaching `Maximum` scrolled a custom-drawn view a whole page past its content (`SMP-48`). A wheel notch moves one `SmallChange` and finishes with `EndScroll`; `Delta` arrives in units of 120, and multiplying by it moved 120 × `SmallChange`, which slammed any range under 120 end to end (`SMP-49`). Partial deltas from a precision wheel or trackpad accumulate rather than being dropped. `Scroll` carries the specific `ScrollEventType` for every path and a handler can veto by rewriting `e.NewValue`. A programmatic `Value = Maximum` is still accepted, as upstream. |
| `SplitContainer` | Partial | Not `ContainerControl`-derived here, so no `ActiveControl`, `AutoValidate`, `ValidateChildren`. **`Orientation` changed meaning** — it is now the direction of the bar, as in WinForms; see [MIGRATION.md](MIGRATION.md#breaking-change-splitcontainerorientation). **Three behavioural changes as of 2026-09-04** (W5.22), each of which had been a value set and ignored: `Panel1MinSize`/`Panel2MinSize` are now the clamp the splitter honours (they were plain auto-properties nothing read, with the working minimum kept under the Majorsilence-only `Panel1MinimumSize`/`Panel2MinimumSize`, which survive as forwarding aliases — so a designer's `Panel1MinSize = 150` was accepted and the bar still dragged to 25); `FixedPanel` is read, so the default `None` keeps the split **proportional** on resize instead of pinning Panel1 — maximising a form no longer hands every extra pixel to Panel2, and `FixedPanel.Panel2` no longer does the opposite of what it says; and `SplitterMoving`/`SplitterMoved` are really raised by a drag, the first cancellable and able to rewrite `SplitX`/`SplitY`, the second once at the end. Two structural fixes in the same pass: `Panel1`/`Panel2` are `SplitterPanel` rather than `Panel` (so `SplitterPanel p = sc.Panel1;` compiles, and `Owner` is reachable), and **the constructor no longer forces `Dock = DockStyle.Fill`** — it is `DockStyle.None` as upstream, with `SplitterDistance` defaulting to 50. A `SplitContainer` that relied on the old implicit `Fill` must now set `Dock` itself. Still stored-only: `IsSplitterFixed` and `SplitterIncrement` (so a "fixed" splitter still drags and there is no keyboard splitter movement, `LAY-06`), `BorderStyle`, and the four `AutoScroll` members. |
| `Splitter` | Partial | **Corrected 2026-09-04** (W5.22); the previous entry filed this under "Implemented — no missing members specific to these types", which was true of the member *list* and badly wrong about the behaviour. `SplitPosition` used to alias the bar's own **thickness**, so `splitter1.SplitPosition = 200` produced a 200-pixel-thick bar instead of a 200-pixel-wide panel; it is now the extent of the sibling the bar is docked against (`-1` when there is none, as upstream's `CalcSplitSize` reports), clamped by `MinSize`/`MinExtra`. The control now actually **resizes that sibling on a drag** — its entire purpose, and previously absent, so the classic `Panel(Dock=Left)` + `Splitter` + `Panel(Dock=Fill)` idiom showed a bar with the correct cursor that moved nothing: no exception, no warning. Works on all four dock edges, and `MinSize`/`MinExtra` are consulted rather than merely stored. `SplitterMoved`/`SplitterMoving` were declared `add { } remove { }`, which discarded every handler — the highest-risk shape, since it compiles and leaks nothing at runtime — and are now real events raised from the drag path. The target is found by matching facing edges, as upstream does, not by z-order. Still stub: `BorderStyle`. Note the Majorsilence-only `Drag` event (a delta) remains, and `SplitContainer` uses it: the container switches the sibling-resizing off for its own bar and applies its own `Panel1MinSize`/`Panel2MinSize` arithmetic instead. |
...
| `MenuStrip`, `ContextMenuStrip` — `Show` coordinates and strip events | Partial | Fixed 2026-09-03 (`W5.16`). `Show (Control, Point)` takes a point in the control's **client** coordinates, as upstream does, so `contextMenuStrip1.Show (button1, new Point (0, button1.Height))` opens under the button rather than at the top-left of the screen (`TSM-03`, P0); `Show (Point)` remains screen-space. `ItemAdded`, `ItemRemoved` and `ItemClicked` now fire on menus, not only on a plain `ToolStrip` — they were wired inside an items facade that `MenuStrip` and `ContextMenuStrip` bypass, so the common single-handler pattern `contextMenuStrip.ItemClicked += ...` never ran (`TSM-08`). `MenuStrip.MenuActivate`/`MenuDeactivate`, `ContextMenu.Popup` (before `Opening`, which is where an app enables items just before display) and `Collapse`, and `MenuItem.Popup` are raised (`TSM-30`). `ShortcutKeys`, the legacy `Shortcut` and access keys have worked since Phase 1's `W1.3` (`TSM-02`, P0, closed then and verified by `MenuShortcutTests.cs`). Still missing: keyboard *navigation* of an open menu — F10/Alt to enter the bar, arrows, Enter, Esc (the remaining half of `TSM-13`) — and `DropDownItemClicked`. |
| `ToolStripMenuItem`, `ToolStripButton`, `ToolStripLabel`, `ToolStripComboBox`, `ToolStripTextBox`, `ToolStripSeparator`, `ToolStripDropDownButton`, `ToolStripSplitButton`, `ToolStripStatusLabel`, `ToolStripProgressBar`, `ToolStripDropDown` | Partial | Core `ToolStripItem` surface (`Text`, `Image`, `Click`, `Enabled`, `Visible`) present; `ToolStripMenuItem.DropDown` is a real `ToolStripDropDown` view onto the item — its `Items` are the same collection as `DropDownItems` and its `Visible` reports whether the sub-menu is open, and setting it opens/closes the sub-menu. Missing accessibility (`AccessibilityObject`), drag/drop (`DoDragDrop`, `DragEnter`/`DragDrop`), and layout internals (`ContentRectangle`, `DefaultMargin`/`DefaultPadding`, `Placement`). Fixed 2026-09-03 (`W5.15`): the `new`-shadowed `Enabled`, `Checked`, `Tag` and `Alignment` are gone, so `MenuItem` — the property the renderers, the click gate and the hit-test all read — owns each one. `saveToolStripMenuItem.Enabled = false` now greys the item, refuses hover and refuses the click, including on a drop-down or context menu, whose click path had no `Enabled` gate of its own at all (`TSM-01` P0, and `TSM-14`, which is the precondition for it). A checked menu item draws a glyph in the image gutter and a checked `ToolStripButton` draws pressed (`TSM-06`); a hidden item is excluded from `ToolBar` layout and painting instead of becoming a dead visible button (`TSM-04`); and `Size`/`Height`/`DisplayStyle` invalidate, so an assignment reaches the box that is drawn (`TSM-31`). `EnabledChanged`, `VisibleChanged`, `AvailableChanged` and `DisplayStyleChanged` are raised. Still missing: accessibility, drag/drop, layout internals, and the appearance knobs the renderers still ignore (`ForeColor`/`Font`, `Spring`, `ShortcutKeyDisplayString`, `ToolTipText`). |
...
| `ErrorProvider` | Partial | Renders as of 2026-09-04 (`W5.20d`). `SetError` draws an error glyph beside the control, honouring `SetIconAlignment` and `SetIconPadding`; `Clear` and an empty description remove it, and a hidden control shows none. Previously **nothing rendered at all** — `SetError` wrote to a dictionary only `GetError` read, so the canonical `errorProvider1.SetError (txtName, "Required")` in a `Validating` handler gave the user no feedback and a form refused to submit with nothing on screen explaining why (`SMP-51`, P0). The icon is painted by the control's parent through `Control.PaintAdorners`, a layer that runs after the container's children so the glyph sits on top of them; upstream uses a separate `ErrorWindow` per control, which this framework has no child windows for. **Still missing:** blinking (`BlinkStyle`/`BlinkRate` are honoured as state, no timer runs), a custom `Icon` — the built-in glyph is always drawn — and the hover tooltip. |
