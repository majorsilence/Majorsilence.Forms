...
| `Metafile`/`MetafileHeader`/`EmfType`/`EmfPlusRecordType`/`MetaHeader`/`WmfPlaceableFileHeader`, Win32 handle interop (`H*`/`hdevmode`/`hdevnames`), the design-time converters | Present; partly functional | *Updated 2026-08-04 — all of these existed as non-goals until the drawing baseline reached zero.* The converters (`FontConverter`, `IconConverter`, `ImageConverter`, `ImageFormatConverter`, `MarginsConverter`) are fully functional and round-trip. So are the metafile headers, `Font.FromLogFont`/`ToLogFont`, `Region.GetRegionData` and `Metafile.GetMetafileHeader`, which parses real EMF and placeable-WMF bytes. What cannot work throws `PlatformNotSupportedException` naming the member and the alternative: every member that would have to produce or read a Win32 handle, and the metafile *recording* constructors. A zero handle would have been worse than absence — it corrupts silently in the caller's next P/Invoke — and absence stopped the whole file compiling. See [the GDI plan](docs/gdi-gap-plan.md#phase-9--the-non-goals-reconsidered). |
