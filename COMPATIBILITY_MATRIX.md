...
| `RichTextBox` | Partial | No `Undo`/`Redo`/`CanUndo`/`CanRedo`/`RedoActionName`, no `SelectedRtf`, no `CanPaste`, no `AutoWordSelection`. As of 2026-09-03 (`W5.12`) it no longer shadows `AppendText` and `SelectedText` with `new`: those shadows assigned `Text`, so appending or replacing a selection scrolled to the top, cleared undo and reset `Modified` — and only when called through a `RichTextBox`-typed reference, so the same object behaved two different ways (`TXT-02` P0, `TXT-35`). `ScrollBars` is still a `new` shadow and still stored-only (W5.14). |
