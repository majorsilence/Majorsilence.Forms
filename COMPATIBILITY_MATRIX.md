...
| `ImageAnimator` | Partial (single frame) | The type and all its methods exist, but `CanAnimate` always returns false and the animate hooks are documented no-ops: there is no multi-frame decode path yet (`FrameDimension`/`Image.GetFrameCount`/`SelectActiveFrame` are missing, see the `FrameDimension` row). Animated GIFs render as their first frame. Static images are unaffected. |
