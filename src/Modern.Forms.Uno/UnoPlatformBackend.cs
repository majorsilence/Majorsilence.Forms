...
{
    /// <summary>
    /// An <see cref="IPlatformBackend"/> that hosts Modern.Forms on Uno Platform's Skia desktop
    /// target. Modern.Forms does its own SkiaSharp drawing; this backend presents each window through
    ...
    public sealed class UnoPlatformBackend : IPlatformBackend
    {
        ...
        private DispatcherQueue Dispatcher
            => _dispatcher ??= DispatcherQueue.GetForCurrentThread ()
               ?? throw new InvalidOperationException ("No Uno DispatcherQueue on this thread. The Uno app head must start the application before Modern.Forms creates windows.");
