...
namespace Continuum.Forms.Backends
{
    ...
    /// Avalonia backend (<c>Continuum.Forms.Avalonia</c> assembly) when present. Assign a different
    /// backend (e.g. a Uno backend) before the first window is created or
    /// <see cref="Continuum.Forms.Application.Run(Form)"/> is called.
    /// </summary>
    public static class Platform
    {
        ...
        private const string DefaultBackendTypeName = "Continuum.Forms.Backends.AvaloniaPlatformBackend, Continuum.Forms.Avalonia";
        ...
        private static IPlatformBackend ResolveDefaultBackend ()
        {
            ...
            if (type is null)
                throw new InvalidOperationException (
                    "No platform backend is configured. Reference a backend package (e.g. Continuum.Forms.Avalonia) " +
                    "or set Continuum.Forms.Backends.Platform.Backend before creating a window.");
